...
function render(root, channels, models) {
  ...
  const visibleModels = (selectedChannel === 'all'
    ...
    .filter(model => !modelSearch || String(model.id).toLowerCase().includes(modelSearch));
    ...
    <section class="panel">
      <div class="toolbar">
        <div class="toolbar-left">
          ...
          <input id="modelSearch" class="input" type="search" placeholder="搜索模型" value="${escapeHtml(modelSearch)}">
        </div>
      </div>
    </section>
  ...
  const search = root.querySelector('#modelSearch');
  search.addEventListener('input', () => {
    modelSearch = search.value.trim().toLowerCase();
    render(root, channels, models);
  });
}
